...
## Como gerar um .exe (com PyInstaller)

```bash
pip install pyinstaller
```

**Primeiro, gere uma versão COM console** (mais fácil de debugar se algo der errado):

```bash
pyinstaller --onefile --uac-admin --name eunetxinhu-otimizacoes main.py
```

- `--onefile`: gera um único `.exe`, sem pasta de dependências soltas
- `--uac-admin`: coloca a exigência de administrador direto no manifesto do `.exe`.
  O Windows já mostra a janela do UAC antes mesmo de abrir o programa — por isso,
  quando rodando como `.exe`, a lógica de "relançar a si mesmo" em `elevation.py`
  nem chega a ser usada (o `is_admin()` já retorna `True` de cara).

O executável fica em `dist/eunetxinhu-otimizacoes.exe`. Testa ele assim algumas vezes.

**Depois de confirmar que funciona sem erros**, gere a versão final sem a janela
de console atrás (mais limpa pra distribuir):

```bash
pyinstaller --onefile --windowed --uac-admin --name eunetxinhu-otimizacoes main.py
```
...
Pra distribuição real (fora do seu PC), o ideal é assinar o executável digitalmente,
o que reduz bastante esses avisos.

### Ícone personalizado (opcional)

Se quiser um ícone próprio no `.exe`, coloque um arquivo `.ico` na pasta do projeto
e adicione `--icon=caminho\para\icone.ico` no comando do PyInstaller.
